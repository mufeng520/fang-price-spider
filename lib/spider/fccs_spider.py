...
class LouPanBaseSpider(BaseSpider):
    ...
    def get_loupan_info(city_name):
        ...
        # 获得总的页数
        try:
            page_box = soup.find_all('span', class_='w_c_1 fb')[0]
            matches = re.search('<span class="w_c_1 fb">(\d+)</span>', str(page_box))
            total_page = int(math.ceil(int(matches.group(1)) / 20))
        ...
        for i in range(1, total_page + 1):
            page = 'http://{0}{1}search/p{2}.html'.format(city_name, LOUPAN_URL, i)
            print(page)
